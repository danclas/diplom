...
def statistics(y_true, y_pred):
    ...
    for j in range(y_pred.shape[0]):
        for k in range(1):
            p1, qrs1, t1 = change_mask(y_pred[j][k])
            p2, qrs2, t2 = change_mask(y_true[j][k])
    ...
    for index in df_res.columns:
        ...
        df_res.at['F1', index] = 2 * df_res.loc['Se', index] * df_res.loc['PPV', index] / (df_res.loc['Se', index] + 
                                                                                           df_res.loc['PPV', index])
